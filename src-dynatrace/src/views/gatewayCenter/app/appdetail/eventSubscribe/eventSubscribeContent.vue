<template>
  <div class="editPanel">
    ...
    <el-form class="default-width" :model="ruleForm" :rules="rules" ref="ruleForm" label-position="top">
      ...
      <p v-show="ruleForm.subscribe_code" class="tilte el-form-item__label">自定义请求头</p>
      <div v-show="ruleForm.subscribe_code && ruleForm.headers && ruleForm.headers.length" class="headerList mb16">
      ...
      <div class="el-button-form-item" v-show="ruleForm.subscribe_code"><DYButton @click="addHeader">添加</DYButton></div>
      ...
      <div class="btns" v-permission="'gatewayCenter_appDetail_eventSubscribeList_edit'" v-if="!ruleForm.record_id">
        <DYButtonGroup>
          <DYButton type="primary" @click="submitForm('ruleForm')">保存</DYButton>
          <DYButton @click="resetForm('ruleForm')">取消</DYButton>
        </DYButtonGroup>
      </div>
      <div class="btns" v-permission="'gatewayCenter_appDetail_eventSubscribeList_add'" v-else>
        <DYButtonGroup>
          <DYButton type="primary" @click="submitForm('ruleForm')">保存</DYButton>
          <DYButton @click="resetForm('ruleForm')">取消</DYButton>
        </DYButtonGroup>
...
export default {
  ...
  computed: {
    _category () {
      if (this.clientDetails && this.clientDetails.category) { return this.clientDetails.category } return this.category
    }
  },
  methods: {
    addHeader () {
