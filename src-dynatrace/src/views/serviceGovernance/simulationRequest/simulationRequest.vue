<template>
  <div class="simulationRequest">
    ...
    <div class="contentSimulation">
      <div class="down plr20 pt19 pb40">
        <div class="head_title_l flex-start pb40">
          <div class="bannericon iconfont iconrequest"></div>
          <div class="title ml8">
            <div class="h1">模拟请求</div>
            <div class="h2">通过选择服务接口，并发送携带固定请求Header的请求，对于应用功能本身不会产生实际调用。通过请求的返回结果，判断服务路由规则是否生效以及配置正确。</div>
          </div>
        </div>
        <div class="desc default-label"></div>
        <el-form ref="ruleForm" label-position="top">
          <div>
            <div class="mb10">已添加接口</div>
            <span class="tag_item tag_item_reset" v-for="(item,index) in endpointListAdd" :key="index">
              <span class="tag_val" v-if="item||item==0">{{item.name}}</span>
              <span class="tags_remove" v-if="item||item==0" @click="remove(index)"><i class="el-icon-close"></i></span>
            </span>
          </div>
          <el-button class="btn conteniu" type="primary" @click="addEndponit">继续添加</el-button>
          <el-form-item class="mb-30" label="发送方式">
            <el-select
              style="width: 150px"
          ...
          <el-form-item class="mb-30" label="路由标签匹配">
            <div clasa="mb12 sign">以key:value形式创建，key/value规则：长度1-63之间，支持字母，连字符(‘-’)，下划线（‘_’），点号（‘.’）数字。以字母开头,，以字母或数字结尾</div>
              ...
              <div class="sf-tagsinput" v-if="editData.match_type">
                <div class="tagsinput_values">
                  <span class="tag_item" v-for="(item, index) in editData.match_value" :key="index">
                    ...
                    <span class="tags_remove" v-if="item||item==0" @click="removeTag(index)"><i class="el-icon-close"></i></span>
                  </span>
          ...
          <el-button class="btn" style="width: 90px;" type="primary" @click="send('single')">发送</el-button>
        </el-form>
      </div>
      <div class="down plr20 pt19 content-bottom" v-show="deploy_groups.length > 0">
        ...
        <div class="flex">
          <div class="flex-left">
            ...
            <div v-for="(item,index) in deploy_groups" :key="index" class="table-head" :style="{background: index%2 === 1 ? '#F8F8F8' :''}">
...
import './index.less'
import { NXMC_MESH_LIST_GET, ENDPOINT_MOCK_POST } from '@/api'
...
export default {
  ...
  methods: {
    ...
    tagsInputkeyupEnter (event) {
      ...
      if (value.indexOf(':') > -1) {
        if (value.split(':').length === 2) {
          let findIndex = this.editData.match_value.findIndex(item => item.key === value.split(':')[0])
          if (findIndex === -1) {
    ...
    // 选择服务列表
    selChange (value) {
      let obj = this.selectData.find(item => item.value === value)
      this.detailData = obj
