<template>
  <div :style="{maxHeight: maxHeight + 'px', top: !tabList.length ? '0' : ''}" v-show="timeShow" class="timePanel"
    ...
    <div class="customTime" v-show="currTab === 'custom'">
      ...
      <nt-calendar :compName="'sd'" :theme="theme" ref="sdCalendar" v-show="showStartDateCalendar" @change="dayChange"></nt-calendar>
      <div v-show="showStartDateCalendar" class="clock flex-start">
        <span class="endlab">开始时间</span>
        <div class="flex-center flex-column">
          <div @click="hourChange(0)" class="iconfont iconarrowup"></div>
          <div class="timeVal">{{sHour}}</div>
          <div @click="hourChange(1)" class="iconfont iconarrowdown"></div>
        </div>
        <div class="midSym">:</div>
        <div class="flex-center flex-column">
          <div @click="minChange(0)" class="iconfont iconarrowup"></div>
          <div class="timeVal">{{sMin}}</div>
          <div @click="minChange(1)" class="iconfont iconarrowdown"></div>
        </div>
      </div>
      ...
      <div class="date-pick-btn">
        <div class="gap"></div>
        <div class="flex-end">
          <el-button @click="confirm('deleteHeadTime')" type="primary">确定</el-button>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    confirm (showDelete) {
      ...
      var st = Date.parse(new Date(s)) + ''
      var et = Date.parse(new Date(e)) + ''
      this.timeShow = false
      let kvs = {
        sYea: this.sYea,
        sMon: this.sMon,
...
<style lang="less" rel="stylesheet/less" scoped>
  ...
  .timePanel {
      ...
      .timeGroup {
        ...
        .currTime {
          border: 2px solid #4FD5E0 !important;
        }
      }
