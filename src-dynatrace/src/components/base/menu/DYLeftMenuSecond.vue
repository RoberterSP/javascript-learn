  ...
  <div class="leftmenu">
    ...
    <div v-if="!item.hide" class="side-nav" v-for="(item, parindex) in leftMenu.menuList" :key="parindex" :class="[parIndex===parindex?'is_par_active':'']">
      <div class="sn_present" @click="presentClick(parindex)" v-if="!!!item.isHide">
        ...
        <div class="sn_right iconfont" v-if="item.open"><DYIcon type="arrowup"></DYIcon></div>
        <div class="sn_right iconfont" v-if="!item.open"><DYIcon type="arrowdown"></DYIcon></div>
      </div>
...
.leftmenu {
  ...
  .side-nav {
    ...
    &.is_par_active{
      &::before{
        border-top: 2px solid @turq-06;
      }
      &::after{
        border-bottom: 2px solid @turq-06;
      }
    }
    .sn_present {
      display: flex;
